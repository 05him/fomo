...
export const BookMarks = () => { 
    ...
    //     setPostToSHow(postCall.data.posts.filter( ({username}) => username===loggedInUserData.username || followingList.includes(username) ).toSorted( (a,b) => {
    //         if(sortByLatest){
    ...
    const fetchPosts = () => {
        if(postToShow.length===0 && loggedInUserData.bookmarks.length!==0 ){
            setPostToSHow(loggedInUserData.bookmarks);
        }
    }
    ...
    <CreatePost />
            ...
            {
                postToShow.length===0 && <div> No BookMarks </div>
            }
        </section>
